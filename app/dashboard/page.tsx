...
// Remplace ceci par ta vraie fonction de récupération des holerites utilisateur
async function getUserPayslips(userId?: string) {
  if (!userId) return [];
  // Exemple :
  // const { data } = await supabase.from('holerites').select('*').eq('user_id', userId);
  // return data || [];
  return [];
}
