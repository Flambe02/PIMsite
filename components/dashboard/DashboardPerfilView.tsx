    ...
    <div className="w-full relative">
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-8 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-base">
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Nome</label>
            <input id={id} type="text" className="input" value={core.nome} onChange={e => setCore(c => ({ ...c, nome: e.target.value }))} placeholder="Seu nome" />
          </div>
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Nome preferido</label>
            <input id={id} type="text" className="input" value={core.nomePreferido} onChange={e => setCore(c => ({ ...c, nomePreferido: e.target.value }))} placeholder="Como prefere ser chamado" />
          </div>
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Data de nascimento</label>
            <input id={id} type="date" className="input" value={core.nascimento} onChange={e => setCore(c => ({ ...c, nascimento: e.target.value }))} />
          </div>
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Gênero</label>
            <select id={id} className="input" value={core.genero} onChange={e => setCore(c => ({ ...c, genero: e.target.value }))}>
          ...
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Cidade/Estado</label>
            <input id={id} type="text" className="input" value={core.cidade} onChange={e => setCore(c => ({ ...c, cidade: e.target.value }))} placeholder="Onde mora" />
          </div>
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-8 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-base">
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Tipo de vínculo</label>
            <select id={id} className="input" value={status.tipoVinculo} onChange={e => setStatus(s => ({ ...s, tipoVinculo: e.target.value }))}>
              <option value="">Selecione</option>
          ...
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Cargo</label>
            <input id={id} type="text" className="input" value={status.cargo} onChange={e => setStatus(s => ({ ...s, cargo: e.target.value }))} placeholder="Cargo ou função" />
          </div>
          {((status.tipo || raw.profile_type) === "CLT") && <>
            <div>
              <label htmlFor={id} className="font-semibold block mb-1">Data de entrada na empresa</label>
              <input id={id} type="date" className="input" value={status.dataEntrada} onChange={e => setStatus(s => ({ ...s, dataEntrada: e.target.value }))} />
            </div>
            <div>
              <label htmlFor={id} className="font-semibold block mb-1">Número de matrícula</label>
              <input id={id} type="text" className="input" value={status.matricula} onChange={e => setStatus(s => ({ ...s, matricula: e.target.value }))} placeholder="Matrícula" />
            </div>
          </>}
          {((status.tipo || raw.profile_type) === "PJ") && <>
            <div>
              <label htmlFor={id} className="font-semibold block mb-1">Tipo de imposição</label>
              <select id={id} className="input" value={status.tipoImposicao} onChange={e => setStatus(s => ({ ...s, tipoImposicao: e.target.value }))}>
                <option value="">Selecione</option>
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-8 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Nome da empresa</label>
            <input id={id} type="text" className="input" value={empresa.nome} onChange={e => setEmpresa(emp => ({ ...emp, nome: e.target.value }))} placeholder="Empresa onde trabalha" />
          </div>
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Setor</label>
            <select id={id} className="input" value={empresa.setor} onChange={e => setEmpresa(emp => ({ ...emp, setor: e.target.value }))}>
              <option value="">Selecione</option>
              {setoresList.map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Localização</label>
            <input id={id} type="text" className="input" value={empresa.localizacao} onChange={e => setEmpresa(emp => ({ ...emp, localizacao: e.target.value }))} placeholder="Cidade/Estado da empresa" />
          </div>
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Tamanho</label>
            <select id={id} className="input" value={empresa.tamanho} onChange={e => setEmpresa(emp => ({ ...emp, tamanho: e.target.value }))}>
              <option value="">Selecione</option>
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-8 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Email</label>
            <input id={id} type="email" className="input" value={comunicacao.email} onChange={e => setComunicacao(c => ({ ...c, email: e.target.value }))} placeholder="Seu email" />
            <select id={id} className="input mt-1" value={comunicacao.tipoEmail} onChange={e => setComunicacao(c => ({ ...c, tipoEmail: e.target.value }))}>
              <option>Pessoal</option>
              <option>Profissional</option>
            </select>
          </div>
          <div>
            <label htmlFor={id} className="font-semibold block mb-1">Telefone</label>
            <input id={id} type="tel" className="input" value={comunicacao.telefone} onChange={e => setComunicacao(c => ({ ...c, telefone: e.target.value }))} placeholder="Seu telefone" />
            <select id={id} className="input mt-1" value={comunicacao.tipoTelefone} onChange={e => setComunicacao(c => ({ ...c, tipoTelefone: e.target.value }))}>
              <option>Pessoal</option>
              <option>Profissional</option>
            </select>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-8 mb-8">
        ...
        <select id={id} className="input w-full max-w-xs" value={casa} onChange={e => setCasa(e.target.value)}>
          <option value="">Selecione</option>
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-8 mb-8">
        ...
        <input id={id} type="text" className="input max-w-xs" value={parceiro} onChange={e => setParceiro(e.target.value)} placeholder="Nome do parceiro(a)" />
      </div>
      {/* Section Dependentes */}
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-8 mb-8">
        ...
        <input id={id} type="text" className="input max-w-xs" placeholder="Nome do dependente" onKeyDown={e => {
