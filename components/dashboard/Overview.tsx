...
export default function Overview({ 
  ...
  // Utiliser les vraies recommandations si disponibles, sinon les recommandations par défaut basées sur les données du holerite
  const recommendations = hasHolerite ? 
    (topRecommendations.length > 0 ? topRecommendations : [
      {
        id: 1,
        title: locale === 'br' ? 'Negociação Salarial' : 
               locale === 'fr' ? 'Négociation Salariale' : 
               'Salary Negotiation',
        description: locale === 'br' ? 
          `Com base no seu salário bruto de R$ ${salarioBruto.toLocaleString('pt-BR')} e eficiência de ${eficiencia.toFixed(1)}%, considere negociar um aumento salarial. Sua experiência desde 2017 e contribuições justificam uma reavaliação.` :
          locale === 'fr' ? 
          `Basé sur votre salaire brut de R$ ${salarioBruto.toLocaleString('pt-BR')} et efficacité de ${eficiencia.toFixed(1)}%, considérez négocier une augmentation salariale. Votre expérience depuis 2017 et contributions justifient une réévaluation.` :
      ...
      {
        id: 2,
        title: locale === 'br' ? 'Otimização Fiscal' : 
               locale === 'fr' ? 'Optimisation Fiscale' : 
               'Tax Optimization',
        description: locale === 'br' ? 
          `Seu salário líquido de R$ ${salarioLiquido.toLocaleString('pt-BR')} representa ${((salarioLiquido/salarioBruto)*100).toFixed(1)}% do bruto. Considere deduções fiscais para otimizar sua eficiência salarial.` :
          locale === 'fr' ? 
          `Votre salaire net de R$ ${salarioLiquido.toLocaleString('pt-BR')} représente ${((salarioLiquido/salarioBruto)*100).toFixed(1)}% du brut. Considérez les déductions fiscales pour optimiser votre efficacité salariale.` :
      ...
      <Card className="shadow-lg border-gray-100 rounded-2xl bg-white">
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-full">
            {/* Salário Líquido - Avec informations intégrées */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full mt-6">
            ...
            {/* Deduções Fiscais - Informations détaillées */}
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl shadow-lg border border-blue-200 p-6 h-full">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-lg font-bold text-blue-900">
                    {locale === 'br' ? 'Deduções Fiscais' : 
                     locale === 'fr' ? 'Déductions Fiscales' : 
                     'Tax Deductions'}
                  </h3>
                </div>
                  ...
                  {eficiencia < 70 && (
                    <div className="bg-blue-100 border border-blue-200 rounded-lg p-3">
                      <div className="text-xs text-blue-800 font-medium">
                        💡 {locale === 'br' ? 'Dica: Verifique possíveis deduções fiscais para otimizar sua eficiência salarial.' : 
                             locale === 'fr' ? 'Conseil: Vérifiez les déductions fiscales possibles pour optimiser votre efficacité salariale.' : 
