-- Correction définitive du trigger pour les nouveaux holerites
...
-- 2. Créer la fonction d'extraction robuste
...
DECLARE
  ...
  obj_value jsonb;
BEGIN
  -- Fonction interne pour extraire les valeurs numériques
  CREATE OR REPLACE FUNCTION extract_numeric_from_json_internal(json_data jsonb, field_name text) 
  RETURNS text AS $$
  DECLARE
    extracted_value text;
    obj_value jsonb;
  ...
  IF NEW.structured_data IS NOT NULL THEN
    ...
    extracted_value := extract_numeric_from_json_internal(NEW.structured_data, 'salario_bruto');
    IF extracted_value IS NOT NULL AND extracted_value != '0.00' THEN
      NEW.salario_bruto := extracted_value;
    END IF;
    
    -- Extraire salario_liquido
    extracted_value := extract_numeric_from_json_internal(NEW.structured_data, 'salario_liquido');
    IF extracted_value IS NOT NULL AND extracted_value != '0.00' THEN
  ...
  -- Nettoyer la fonction interne
  DROP FUNCTION IF EXISTS extract_numeric_from_json_internal(jsonb, text);
...
-- 4. Tester le trigger avec un holerite de test
